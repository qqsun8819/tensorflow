...
// 4) bazel-bin/tensorflow/compiler/mlir/tf-opt tensorflow/compiler/mlir/tensorflow/tests/mlir_runtime_results.mlir -tf-executor-to-tf-conversion --xla-legalize-tf -hlo-legalize-to-lhlo --lhlo-legalize-to-linalg -convert-tf-to-affine  --lhlo-legalize-to-std --convert-linalg-to-affine-loops  --lower-affine  --convert-loop-to-std --convert-std-to-llvm | bazel-bin/tensorflow/compiler/mlir/tensorflow/tf-mlir-runtime -e main_ret2 -entry-point-result=user_define -shared-libs=/home/jiankeng.pt/workspace/tensorflow-bak/bazel-bin/tensorflow/compiler/mlir/libtf_mlir_external_copy_results.so
...
module attributes {tf.versions = {bad_consumers = [], min_consumer = 0 : i32, producer = 134 : i32}} {
  func @main_ret1(%arg0: memref<?xi64>, %arg1: i64) {
    tf_executor.graph {
      %src1, %src_ctl1 = tf_executor.island wraps "tf.Const"() {name = "Const", value = dense<[[8,5], [9,3]]> : tensor<2x2xi32>} : () -> tensor<?x?xi32>
      %dest_ct2 = tf_executor.island wraps "tf.CopyResult"(%arg1, %src1) {} : (i64, tensor<?x?xi32>) -> ()
      tf_executor.fetch
    }
    return
  }

  func @main_ret2(%arg1: i64) {
    tf_executor.graph {
      ...
      %dest = tf_executor.island wraps "tf.CopyResult"(%arg1, %sub) {} : (i64, tensor<?x?xi64>) -> ()
      tf_executor.fetch
    }
    return
  }

  func @main_ret3(%arg0: memref<?x?xf64>, %arg1: memref<?x?xf64>, %ret0: i64) {
    tf_executor.graph {
      %dest_ctl = tf_executor.island wraps "tf.CopyResult"(%ret0, %arg0) {} : (i64, memref<?x?xf64>) -> ()
      tf_executor.fetch
    }
    return
  }
